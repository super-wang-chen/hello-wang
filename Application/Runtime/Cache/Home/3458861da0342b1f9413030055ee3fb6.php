	...
	<div class="main">
		<div class="container">
			...
			<div class="content">
				<!-- 副导航开始 -->
				<div class="subnav">
					<?php if(is_array($cate_all)): $i = 0; $__LIST__ = $cate_all;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="/qmss/index.php/Home/Product/index?cate=<?php echo ($vo["id"]); ?>&p=1" ><?php echo ($vo["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
				<!-- 副导航结束 -->
...
$(function(){
	...
	$(".subnav a").eq(0).addClass("selected");
	$(".subnav a").click(function(){
		$(".subnav a").removeClass();
		$(this).addClass("selected");
	});
    $(".page div a,.subnav a").live('click',function(){
      //alert(1);
      ...
      $.ajax({
        ...
        success:function(msg){
          ...
          for(var i=0;i<msg.lists.length;i++){
			...
			str+='</a>';
			str+='<a href="';
			str+="<?php echo U('Shopping/add_cart');?>?id="+msg.lists[i]['id'];
			str+='" class="add-cart">加入购物车</a>';
			str+='</div>';
			str+='</li>';
          }
          $(".tab ul").html(str);
